...
usage() {
	cat <<EOF
Usage: $prog [-d DIR] [-s SRCDIR] [-c LIST] [-j N] [-x ARGS] [-h]
...
native Windows builds (nmake / MSBuild) use gen-build-matrix.ps1 instead.
  ...
  -x ARGS    Extra configure arguments appended to every cell, e.g.
             "--enable-unit-tests". They are added after the cell's own
             arguments, so a cell that already sets an option keeps it.
             Nothing here is probed for - if the option needs a library that
             is not installed, every cell fails to configure.
...
while getopts "d:s:c:j:x:h" opt; do
	case "$opt" in
		d)
			master=$OPTARG
			;;
...
# Kept as a single pre-spaced string rather than appended at each use, so that
# the empty case adds nothing at all - an extra space would end up in the
# generated build.sh and in matrix-info.txt, and make an unchanged run look
# changed to anyone diffing them.
extra_suffix=
...
for d in $detected; do
	...
	cells | while IFS='|' read -r cell args; do
		...
		mkdir -p "$celldir"
		{
			...
			echo "# configure args: $args$extra_suffix"
			echo "set -e"
			...
			echo "\"\$srcdir/configure\" $args$extra_suffix"
			echo "make -j$jobs"
		} > "$celldir/build.sh"
		chmod +x "$celldir/build.sh"
		echo "  [gen]  $cname/$cell : $args$extra_suffix" >> "$info"
	done
done
